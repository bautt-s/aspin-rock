...
const ArticleDetail = () => {
    const [details, setDetails] = useState<articleInterface>()
    const dispatch = useDispatch()
    const router = useRouter()

    const getDetails = async () => {
        const response = await fetch(`/api/articles?id=${router.query.id}`, {
            method: "GET"
        })
                    ...
                    <div className="flex flex-col px-[40px] md:px-[80px] lg:px-[220px] mt-[60px] pb-[120px]">
                        ...
                        <div className="flex flex-row gap-x-[60px] mt-[60px]">
                            ...
                            <span className="text-gray-400 font-[300]">
                                TOPIC: <strong className="text-[#1DA1F2] font-[300]">{details?.category.toUpperCase() || ' INFORMATIVE'}</strong>
                            </span>
                        </div>
